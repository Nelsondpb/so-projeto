...
SRCS = src/config.c src/games.c src/logs.c src/verify.c src/protocol.c src/solver.c src/client_ui.c src/duel.c
...
servidor: $(SRCS) $(SERVER_SRC)
	$(CC) $(CFLAGS) -pthread -o servidor $(SERVER_SRC) $(SRCS)
	@echo "\033[1;36m--> servidor criado\033[0m"

cliente: $(SRCS) $(CLIENT_SRC)
	$(CC) $(CFLAGS) -pthread -o cliente $(CLIENT_SRC) $(SRCS)
	@echo "\033[1;36m--> cliente criado\033[0m"

test: all
	@echo "\033[1;33mExecutar ./servidor servidor.conf e ./cliente cliente.conf para demo\033[0m"
