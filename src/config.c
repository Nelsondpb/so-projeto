...
int load_server_config(const char *path, ServerConfig *cfg) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strcmp(key, "JOGOS_FILE") == 0) strncpy(cfg->jogos_file, value, MAX_PATH-1);
        else if (strcmp(key, "LOG_FILE") == 0) strncpy(cfg->log_file, value, MAX_PATH-1);
...
int load_client_config(const char *path, ClientConfig *cfg) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (parse_key_value(line, key, value) != 0) continue;
        for (char *p = key; *p; ++p) *p = (char)toupper((unsigned char)*p);
        if (strcmp(key, "SERVER_IP") == 0) strncpy(cfg->server_ip, value, sizeof(cfg->server_ip)-1);
        else if (strcmp(key, "SERVER_PORT") == 0) cfg->server_port = atoi(value);
        else if (strcmp(key, "CLIENT_ID") == 0) cfg->client_id = atoi(value);
        else if (strcmp(key, "LOG_FILE") == 0) strncpy(cfg->log_file, value, MAX_PATH-1);
    }
    fclose(f);
    return 0;
