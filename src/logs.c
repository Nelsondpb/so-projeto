...
/*
 * Garante a existência da diretoria "logs".
 *
 * Se a diretoria não existir, é criada automaticamente.
 * Evita erros ao tentar escrever ficheiros de log.
 *
 * Parâmetros:
 *  - path : caminho base para os logs (não usado diretamente)
 */
 ...
 *  - userId  : identificador do cliente/servidor
...
int log_event(const char *logfile, int userId, const char *event, const char *desc) {
    ...
    /* Se o caminho já contiver diretoria, usar diretamente */
